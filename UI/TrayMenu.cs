...
namespace HotCorners.UI;

// Themed system-tray context menu in the same WinUI 3 style as Swoosh: rounded
// hover pill, rounded window corners (DWM), and a palette that swaps to match
// the OS light/dark setting every time the menu opens. It is a real WinForms
// ContextMenuStrip so Windows owns the modal menu loop — we only restyle it.
internal static class TrayMenu
{
        ...
        out ToolStripMenuItem pauseItem)
    {
        ...
        var settings = NewItem("Hot Corners Settings\u2026", onSettings);

        var pause = NewItem("Pause", onTogglePause);
        pause.CheckOnClick = false;
        pauseItem = pause;

        var check = NewItem("Check for Updates\u2026", onCheckUpdates);

        var about = NewItem("About Hot Corners\u2026", onAbout);

        var quit = NewItem("Exit", onQuit);
        ...
        // Re-read the OS theme + state each time the menu opens so it tracks live.
        menu.Opening += (_, _) =>
        {
    ...
    // ContextMenuStrip auto-measures item height from the font/image and largely
    // ignores Padding, so we add vertical breathing room by overriding the
    // preferred size directly.
